...
public class ModuleSettingsScreen extends Screen {
    ...
    protected void init() {
        ...
        if (module instanceof ESP esp) {
            ...
            this.addRenderableWidget(net.minecraft.client.gui.components.CycleModernButtonBuilder.create(
            ...
            this.addRenderableWidget(ModernButtonBuilder.create(
        ...
        if (module instanceof Velocity vel) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Horizontal KB", 0.0, 1.0,
                    vel.getHorizontalPercent(), val -> vel.setHorizontalPercent(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Vertical KB", 0.0, 1.0,
                    vel.getVerticalPercent(), val -> vel.setVerticalPercent(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Chance", 0.0, 1.0,
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            if (vel.isPulseMode()) {
                components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Pulse Interval", 1, 100,
                        vel.getPulseInterval(), val -> { vel.setPulseInterval((int) val); init(); }));
                y += ROW_HEIGHT + ROW_SPACING;
            }
            this.addRenderableWidget(ModernButtonBuilder.create(Component.literal("Mode: " + vel.getMode().getLabel()), b -> {
                vel.cycleMode();
                init();
            }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
        ...
        if (module instanceof NoHitDelay nhd) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Delay (ticks)", 0, 10,
                    nhd.getDelayTicks(), val -> nhd.setDelayTicks((int) val)));
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(ModernButtonBuilder.create(Component.literal("Preset: " + nhd.getPreset().getLabel()), b -> {
                nhd.cyclePreset();
                init();
            }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
        ...
        if (module instanceof SilentAura sa) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Min CPS", 1.0, 20.0,
                    sa.getMinCps(), val -> sa.setMinCps(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Max CPS", 1.0, 20.0,
                    sa.getMaxCps(), val -> sa.setMaxCps(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Smoothing", 1.0, 10.0,
                    sa.getSmoothing(), val -> sa.setSmoothing(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Snapiness", 1.0, 10.0,
                    sa.getSnapiness(), val -> sa.setSnapiness(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Attack Range", 2.0, 6.0,
                    sa.getAttackRange(), val -> sa.setAttackRange(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Max Angle", 10.0, 360.0,
                    sa.getMaxAngle(), val -> sa.setMaxAngle(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Target: " + sa.getTargetMode().getLabel()), b -> {
                        sa.cycleTargetMode();
                        init();
                    }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
        ...
        if (module instanceof AutoTotem at) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Health Threshold", 0, 20,
        ...
        if (module instanceof AutoTools at) {
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Priority: " + at.getAttackPriority().getLabel()), b -> {
        ...
        if (module instanceof ChestESP ce) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Range", 8.0, 128.0,
                    ce.getRange(), val -> ce.setRange(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            addFilterRow(centerX, y, ce::isChestsEnabled, ce::setChestsEnabled, "Chests");
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Chest Color: " + ce.getChestsColor().getLabel()), b -> {
                        ce.cycleChestsColor();
                        init();
                    }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Ender Color: " + ce.getEnderChestsColor().getLabel()), b -> {
                        ce.cycleEnderChestsColor();
                        init();
                    }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Trapped Color: " + ce.getTrappedChestsColor().getLabel()), b -> {
        ...
        if (module instanceof OreESP oe) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Diamond Color: " + oe.getDiamondColor().getLabel()), b -> {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Gold Color: " + oe.getGoldColor().getLabel()), b -> {
                        oe.cycleGoldColor();
                        init();
                    }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Emerald Color: " + oe.getEmeraldColor().getLabel()), b -> {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Redstone Color: " + oe.getRedstoneColor().getLabel()), b -> {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Lapis Color: " + oe.getLapisColor().getLabel()), b -> {
                        oe.cycleLapisColor();
                        init();
                    }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Coal Color: " + oe.getCoalColor().getLabel()), b -> {
                        oe.cycleCoalColor();
                        init();
                    }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Debris Color: " + oe.getAncientDebrisColor().getLabel()), b -> {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("N.Gold Color: " + oe.getNetherGoldColor().getLabel()), b -> {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(
                    ModernButtonBuilder.create(Component.literal("Quartz Color: " + oe.getQuartzColor().getLabel()), b -> {
                        oe.cycleQuartzColor();
                        init();
                    }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
        ...
        if (module instanceof BedESP be) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(ModernButtonBuilder.create(Component.literal("Color: " + be.getColorMode().getLabel()), b -> {
                be.cycleColorMode();
                init();
            }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
        ...
        if (module instanceof ShulkerESP se) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            this.addRenderableWidget(ModernButtonBuilder.create(Component.literal("Color: " + se.getColorMode().getLabel()), b -> {
                se.cycleColorMode();
                init();
            }).bounds(centerX - 80, y, 160, ROW_HEIGHT).build());
            y += ROW_HEIGHT + ROW_SPACING;
        ...
        if (module instanceof Reach reach) {
            ...
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Entity reach", 3.0, 8.0,
                    reach.getEntityReach(), val -> reach.setEntityReach(val)));
            y += ROW_HEIGHT + ROW_SPACING;

            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Block reach", 4.5, 10.0,
        ...
        if (module instanceof Criticals crit) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Crit Chance", 0.0, 1.0,
        ...
        if (module instanceof AimAssist aim) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "FOV Limit", 10.0, 360.0,
                    aim.getFov(), val -> aim.setFov(val)));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Distance", 2.5, 6.0,
        ...
        if (module instanceof AutoCrystal ac) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Delay (ticks)", 0.0, 20.0,
        ...
        if (module instanceof TimeChanger tc) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Time", 0, 24000,
                    tc.getTargetTime(), val -> tc.setTargetTime(val)));
            y += ROW_HEIGHT + ROW_SPACING;

            this.addRenderableWidget(ModernButtonBuilder.create(Component.literal("Dawn"), b -> {
                tc.setPresetDawn();
                init();
            }).bounds(centerX - 80, y, 38, ROW_HEIGHT).build());

            this.addRenderableWidget(ModernButtonBuilder.create(Component.literal("Day"), b -> {
                tc.setPresetDay();
                init();
            }).bounds(centerX - 40, y, 38, ROW_HEIGHT).build());
            
            this.addRenderableWidget(ModernButtonBuilder.create(Component.literal("Dusk"), b -> {
                tc.setPresetDusk();
                init();
            }).bounds(centerX, y, 38, ROW_HEIGHT).build());

            this.addRenderableWidget(ModernButtonBuilder.create(Component.literal("Night"), b -> {
        ...
        if (module instanceof SpeedBridge sb) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Auto-off (s)", 0.5, 10.0,
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Delay ticks", 0.0, 4.0,
        ...
        if (module instanceof AutoGG gg) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Delay (ms)", 0.0, 5000.0,
        ...
        if (module instanceof WTap wTap) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Trigger delay", 0.0, 250.0,
                    wTap.getTriggerDelayMs(), val -> wTap.setTriggerDelayMs((int) Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Release delay", 0.0, 250.0,
                    wTap.getReleaseDelayMs(), val -> wTap.setReleaseDelayMs((int) Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Re-press delay", 0.0, 250.0,
                    wTap.getRepressDelayMs(), val -> wTap.setRepressDelayMs((int) Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Cooldown", 0.0, 1000.0,
        ...
        if (module instanceof Triggerbot triggerbot) {
            ...
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Extra delay", -4.0, 10.0,
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Miss chance", 0.0, 1.0,
                    triggerbot.getTargetMissChance(), triggerbot::setTargetMissChance));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Early hit chance", 0.0, 1.0,
        ...
        if (module instanceof HitSelect hitSelect) {
            ...
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Pause Duration", 0.0, 1000.0,
                    (double)hitSelect.getPauseDurationMs(), val -> hitSelect.setPauseDurationMs((int)Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;

            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Wait First Hit", 0.0, 1000.0,
                    (double)hitSelect.getWaitForFirstHitMs(), val -> hitSelect.setWaitForFirstHitMs((int)Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;

            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Combat Rate %", 0.0, 100.0,
                    hitSelect.getInCombatCancelRate(), hitSelect::setInCombatCancelRate));
            y += ROW_HEIGHT + ROW_SPACING;

            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Missed Rate %", 0.0, 100.0,
        ...
        if (module instanceof BowAimbot bowAim) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "FOV", 1.0, 360.0,
                    bowAim.getFov(), bowAim::setFov));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Distance", 10.0, 100.0,
        ...
        if (module instanceof RightClicker rightClicker) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Start delay", 0.0, 1000.0,
                    rightClicker.getStartDelayMs(),
                    val -> rightClicker.setStartDelayMs((int) Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Block place delay", 0.0,
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Jitter", 0.0, 3.0,
        ...
        if (module instanceof JumpReset jumpReset) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Hit Trigger %", 0.0, 1.0,
                    jumpReset.getChance(), jumpReset::setChance));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Jump execution %", 0.0, 100.0,
                    jumpReset.getJumpChancePercent(), jumpReset::setJumpChancePercent));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Accuracy timing", 0.0, 1.0,
                    jumpReset.getAccuracy(), jumpReset::setAccuracy));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Max delay ticks", 0.0, 6.0,
                    jumpReset.getMaxDelayTicks(), val -> jumpReset.setMaxDelayTicks((int) Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Cooldown ticks", 0.0, 20.0,
        ...
        if (module instanceof AntiBot antiBot) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Join grace ticks", 0.0, 200.0,
        ...
        if (module instanceof Health health) {
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Bar Scale", 0.1, 5.0,
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Tag Scale", 0.5, 4.0,
                    health.getTagScale(), val -> health.setTagScale((float) val)));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Distance", 16.0, 160.0,
                    health.getMaxDistance(), health::setMaxDistance));
            y += ROW_HEIGHT + ROW_SPACING;
        }

        if (module instanceof LatencyAlerts latencyAlerts) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "High ping ms", 50.0, 1000.0,
                    latencyAlerts.getHighPingMs(), val -> latencyAlerts.setHighPingMs((int) Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Spike increase", 10.0, 500.0,
                    latencyAlerts.getSpikeIncreaseMs(), val -> latencyAlerts.setSpikeIncreaseMs((int) Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Alert cooldown", 1.0, 60.0,
        ...
        if (module instanceof AutoXPThrow autoXP) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Throws per tick", 1.0, 20.0,
                    autoXP.getThrowsPerTick(), val -> autoXP.setThrowsPerTick((int) Math.round(val))));
            y += ROW_HEIGHT + ROW_SPACING;
        }

        if (module instanceof AntiAFK antiAFK) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Start delay min", 5.0, 300.0,
                    antiAFK.getMinStartDelaySeconds(), antiAFK::setMinStartDelaySeconds));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Start delay max", 5.0, 300.0,
                    antiAFK.getMaxStartDelaySeconds(), antiAFK::setMaxStartDelaySeconds));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Frequency", 0.1, 1.0,
            ...
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Max yaw change", 1.0, 90.0,
                    antiAFK.getMaxYawChange(), antiAFK::setMaxYawChange));
            y += ROW_HEIGHT + ROW_SPACING;
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Max pitch change", 1.0, 90.0,
        ...
        if (module instanceof Indicators indicators) {
            ...
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Radius scale", 0.5, 2.0,
        ...
        if (module instanceof Trajectories traj) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Max Ticks", 30.0, 200.0,
                    traj.getMaxTicks(), traj::setMaxTicks));
            y += ROW_HEIGHT + ROW_SPACING;
            
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Thickness", 1.0, 5.0,
        ...
        if (module instanceof TNTTimer tnt) {
            components.add(new ModernSlider(centerX - 80, y, 160, ROW_HEIGHT, "Scale", 0.5, 3.0,
